...
```{r access-layers-filt, eval=FALSE}
...
```{r norm-comparison, eval=FALSE, fig.alt="Two histograms side by side for the LYZ gene. Left panel shows raw UMI counts with a dominant spike at zero (over 7000 cells) and a low flat tail extending to about 500. Right panel shows log-normalized expression with a spike at zero, a small peak near 0.5-1.0, and a distinct second peak around 4-4.5, revealing a bimodal pattern that separates LYZ-expressing monocytes from non-expressing cells."}
...
```{r variable-plot, eval=FALSE, fig.alt="Scatter plot with average expression on a log-scaled x-axis and standardized variance on the y-axis. 2000 variable features are marked in a distinct style above a dense band of 27155 non-variable genes near the baseline. The top 10 labeled genes include PPBP at the highest variance, followed by PF4, LINC01478, LINC01374, JCHAIN, GP1BB, PTGDS, SOX5, ENSG00000225885, and EREG."}
...
```{r install-glmgampoi, eval=FALSE}
