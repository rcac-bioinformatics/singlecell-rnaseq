...
```{r vlnplot, eval=FALSE, fig.alt="Three violin plots for all cells in the pbmc10k identity group. nFeature_RNA shows a wide bulge centered around 3000-4000 genes with a thin tail down to near zero. nCount_RNA shows most cells below 25000 UMIs with a long right tail extending past 75000. percent.mt shows a dense concentration below 10 percent with a long thin tail reaching above 75 percent."}
VlnPlot(pbmc,
...
```{r scatter, eval=FALSE, fig.alt="Scatter plot with total UMI counts on the x-axis and number of genes on the y-axis, colored by mitochondrial percentage on a grey-to-red gradient. Most cells form a dense curved cloud from lower-left to upper-right, saturating around 7500 genes. A few high-mitochondrial cells appear as darker points in the lower-left. Outliers extend past 75000 UMIs on the right."}
...
```{r vlnplot-after, eval=FALSE, fig.alt="Three violin plots after filtering. nFeature_RNA ranges from about 200 to 7000 with a smooth bulge around 3000-4000. nCount_RNA ranges from near zero to 50000 with most cells below 20000. percent.mt is capped below 15 percent with a smooth unimodal distribution peaking around 5-6 percent."}
VlnPlot(pbmc,
        features = c("nFeature_RNA", "nCount_RNA", "percent.mt"),
        ncol = 3,
        pt.size = 0,
...
```{r scatter-after, eval=FALSE, fig.alt="Scatter plot of total UMI counts versus number of genes after filtering, colored by mitochondrial percentage on a grey-to-red gradient. The cloud is bounded below 50000 UMIs and 7000 genes, with mitochondrial percentage capped below 15 percent. The curved relationship between UMIs and genes is preserved."}
ggplot(pbmc@meta.data, aes(x = nCount_RNA, y = nFeature_RNA, color = percent.mt)) +
    geom_point(size = 0.5, alpha = 0.4) +
    scale_color_gradient(low = "grey90", high = "firebrick", name = "% MT") +
    theme_minimal() +
