...
```{r umap-before, eval=FALSE}
ifnb <- FindNeighbors(ifnb, dims = 1:30, reduction = "pca")
ifnb <- FindClusters(ifnb, resolution = 0.5)
ifnb <- RunUMAP(ifnb, dims = 1:30, reduction = "pca")
```

```{r plot-before, eval=FALSE, fig.alt="UMAP plot of the IFNB dataset without integration. Cells are colored by condition, showing that control and stimulated cells form largely separate clusters rather than mixing by cell type."}
...
```{r plot-after, eval=FALSE, fig.alt="UMAP plot of the IFNB dataset after CCA integration. Control and stimulated cells now intermingle within each cluster, indicating successful batch correction while preserving cell type structure."}
DimPlot(ifnb, reduction = "umap", group.by = "stim")
```
...
```{r split-view, eval=FALSE, fig.alt="Two UMAP panels showing control and stimulated conditions side by side after integration. Both panels show the same cluster structure, confirming that cell types are shared across conditions."}
...
```{r annotated-umap, eval=FALSE, fig.alt="UMAP plot of the integrated IFNB dataset with cells colored by annotated cell type. Major PBMC types including CD4 T, CD8 T, B cells, NK cells, monocytes, and DCs are visible."}
...
```{r isg-feature, eval=FALSE, fig.alt="FeaturePlot showing IFIT3 and IFIT1 expression split by condition. Both genes show strong expression across multiple cell types in the stimulated condition but are nearly absent in the control condition."}
FeaturePlot(ifnb,
...
```{r isg-violin, eval=FALSE, fig.alt="Four violin plots showing IFIT1, IFIT3, TNFSF10, and RSAD2 expression in CD14 monocytes, split by control and stimulated condition. All four genes show dramatically higher expression in the stimulated condition with broad violin shapes peaking around 3 to 4, while control violins are flat near zero."}
VlnPlot(ifnb,
