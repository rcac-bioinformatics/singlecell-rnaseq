...
```{r viz-loadings, eval=FALSE, fig.alt="Dot plots showing the top 30 genes by loading magnitude for PC1 and PC2. PC1 is dominated by myeloid markers including FCN1, LYZ, CST3, S100A9, and S100A8 with negative loadings. PC2 shows B cell markers including BANK1, CD79A, MS4A1, PAX5, and HLA-DQB1 with negative loadings."}
...
```{r elbow, eval=FALSE, fig.alt="Dot plot of standard deviation versus principal component number for 30 PCs. The curve drops steeply from about 12 at PC1 to about 3.5 at PC7, then gradually flattens into a plateau by PC15-20, with an elbow around PC 8 to 10."}
...
```{r umap-plot, eval=FALSE, fig.alt="UMAP plot of 11310 cells all shown in a single color since clustering has not been performed yet. Cells organize into several spatially distinct groups, with large clusters in the upper-left and lower-left, and smaller groups on the right side."}
...
```{r umap-clusters, eval=FALSE, fig.alt="UMAP plot with cells colored by cluster identity at resolution 0.5. Eighteen clusters (numbered 0 through 17) are visible, with large clusters like 0, 1, and 2 occupying broad regions and smaller clusters like 15, 16, and 17 appearing as small isolated groups."}
...
```{r feature-plot, eval=FALSE, fig.alt="Four UMAP panels in a 2x2 grid. CD3D expression highlights multiple clusters across the upper-left and lower-left (T cells). MS4A1 lights up a single compact cluster in the upper-right (B cells). LYZ is strong in the lower-left clusters (monocytes). GNLY marks a small cluster on the right side (NK cells). Expression is shown on a grey-to-purple gradient."}
FeaturePlot(pbmc,
